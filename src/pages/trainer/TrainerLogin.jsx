...
export default function TrainerLogin() {
  ...
  function handleSubmit(e) {
    ...
    setLoading(true)
    setTimeout(() => {
      const result = loginTrainer(email, password)
      setLoading(false)
      if (result.success) {
    ...
    <div className="min-h-screen bg-base-bg flex items-center justify-center px-4 py-10">
      ...
      >
        <div className="flex flex-col items-center mb-6">
          ...
          <h1 className="text-lg font-display font-bold text-ink-primary">Trainer Portal</h1>
        </div>
        ...
        <div className="bg-base-surface border border-base-border rounded-2xl p-6 shadow-card">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <Input
              ...
              type="email"
              required
              placeholder="trainer@tayyebahacademy.com"
              value={email}
          ...
          <p className="text-xs text-ink-muted mt-5 text-center">
            Demo credentials — Email: <span className="text-ink-secondary">{currentTrainer.email}</span> ·
            Password: <span className="text-ink-secondary">{currentTrainer.password}</span>
          </p>
        </div>
