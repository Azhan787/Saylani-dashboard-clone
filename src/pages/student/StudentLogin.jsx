...
export default function StudentLogin() {
  ...
  function handleSubmit(e) {
    ...
    if (!cnic.trim() || !password.trim()) {
      setError('Please fill in both CNIC and password.')
      return
    }
    setLoading(true)
    setTimeout(() => {
      const result = loginStudent(cnic, password)
      setLoading(false)
      if (result.success) {
      ...
      >
        ...
        <div className="bg-base-surface border border-base-border rounded-2xl p-6 shadow-card">
          ...
          <p className="text-sm text-ink-secondary mb-5">
            Kindly provide the CNIC number and password used during Tayyebah Academy course
            registration.
          </p>
          ...
          <p className="text-xs text-ink-muted mt-5 text-center">
            Demo credentials — CNIC: <span className="text-ink-secondary">{currentStudent.cnic}</span> ·
            Password: <span className="text-ink-secondary">{currentStudent.password}</span>
          </p>
        </div>
