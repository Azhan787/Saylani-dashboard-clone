...
import { Mail, Phone, Hash, GraduationCap, Building2, MapPin, Pencil } from 'lucide-react'
...
export default function Profile() {
  ...
  const [form, setForm] = useState({ email: currentStudent.email, phone: currentStudent.phone })
    ...
    <div className="flex flex-col gap-6 max-w-3xl">
      <Card className="p-6 flex flex-col sm:flex-row sm:items-center gap-5">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-xl font-bold text-ink-primary">{currentStudent.fullName}</h2>
          <p className="text-sm text-ink-muted mt-0.5">{currentStudent.course}</p>
        </div>
